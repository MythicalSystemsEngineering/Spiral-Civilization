...
# 2. Write final merged HTML
cat > live/public/index.html <<'EOF'
<!DOCTYPE html>
...
<body>
  ...
  <script>
    ...
    // Tier 3: live data fetch + Mermaid render
    if (tier >= 3) {
      // Meta
      fetch('/meta.json')
        .then(r => r.json())
...
# 3. Append FastAPI endpoints to live/server.py if not present
if ! grep -q "@app.get(\"/graph.mmd\")" live/server.py; then
  echo "Appending endpoints to live/server.py..."
  cat >> live/server.py <<'EOF'
...
@app.get("/graph.mmd")
def get_graph():
    return FileResponse("graph.mmd", media_type="text/plain")
...
# 4. Create starter graph.mmd
cat > graph.mmd <<'EOF'
graph TD
    A[Spiral Core] --> B[Steward Nodes]
    A --> C[Memory Engine]
    B --> D[Active Stewards]
    C --> E[Meta-Cognitive Hooks]
EOF

echo "✅ Setup complete. Run: uvicorn live.server:app --reload"
